                ...
                <div className="tab-content">
                    {activeTab === 'send' && (
                        <Card>
                            ...
                            <button className="btn btn-primary" onClick={handleSend} disabled={loading}>{loading ? "Processing..." : `Send ${sendToken}`}</button>
                    ...
                    {activeTab === 'history' && (
                       <Card>
                         {(historyLoading && displayedHistory.length === 0) ? <p>Loading history...</p> : (
                           <ul className="history-list">
                             {displayedHistory.length > 0 ? displayedHistory.map(tx => {
                                ...
                                return (
                                  <li key={tx.hash} className={clsx({ 
                                    'tx-status-pending': isPending, 
                                    'tx-status-failed': isFailed 
                                  })}>
                                    <div className="tx-icon-and-details">
                                        ...
                                        <div className="tx-details">
                                            ...
                                            {isPending ? (
                                                <p className="status-text pending">Pending</p>
                                            ) : (
                                                <p className="tx-sub-details">{`${txDate.toLocaleDateString()} at ${txDate.toLocaleTimeString()}`}</p>
                                            )}
                                        </div>
                                    </div>
                                    <div className="tx-amount-and-actions">
                                        ...
                                        {isPending ? (
                                            <button className="btn-cancel" onClick={() => handleCancel(tx)} disabled={loading}>
                                                Cancel
                                            </button>
                                        ) : (
                                            <a href={`https://testnet.bscscan.com/tx/${tx.hash}`} target="_blank" rel="noopener noreferrer" className="tx-link">
                                                View
                                            </a>
                                        )}
                                    </div>
                    ...
                    {activeTab === 'contacts' && (
                        <Card title="Address Book">
                            <div className="add-contact-form">
                                ...
                                <div className="input-group">
                                    <input placeholder="Contact Address (0x...)" value={newContactAddress} onChange={(e) => setNewContactAddress(e.target.value)} />
                                </div>
                            ...
                            {showSensitive && (
                                <div className="secrets-box">
                                    <div className="input-group">
                                        ...
                                        <textarea readOnly value={walletData.privateKey} rows={2} />
                                    </div>
                                    <div className="input-group">
                                        <label>Mnemonic Phrase</label>
                                        <textarea readOnly value={walletData.mnemonic} rows={3} />
                                    </div>
                                </div>
                            )}
                        </Card>
